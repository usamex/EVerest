...
void print_status(const chargebridge_status& s) {
    ...
    const std::size_t field_count = fields.size() + 1; // +1 for cb_name
    ...
    os << '|';
    {
        ...
        const std::string name = center(s.cb_name, W);
        os << cb_name_color << name << ansi::reset << '|';
    }
